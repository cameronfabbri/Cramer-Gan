...
def netG(z, batch_size, reuse=False):
   ...
   with tf.variable_scope(sc, reuse=reuse):
      ...
      conv1 = tcl.convolution2d_transpose(z, 512, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv1')
      
      conv2 = tcl.convolution2d_transpose(conv1, 256, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv2')
      
      conv3 = tcl.convolution2d_transpose(conv2, 128, 5, 2, activation_fn=tf.nn.relu, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv3')
...
'''
#def netD(input_images, batch_size, reuse=True):
def netD(input_images, reuse=True):
   ...
   with tf.variable_scope(sc, reuse=reuse):
      ...
      # output is dim 256 as in the paper
      conv4_flat = tcl.flatten(conv4)
      fc = tcl.fully_connected(conv4_flat, 256, activation_fn=tf.identity)
      #conv5 = tcl.conv2d(conv4, 16, 4, 1, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='d_conv5') 
      #return conv5
